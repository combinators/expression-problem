...
object InterleaveStringsToDiskMain extends IOApp {
  ...
  def model:EnhancedModel = {
    ...
    val i1: HelperExpression = HelperExpression("i1", zero, SelfExpression("i1") <= new StringLengthExpression(s1), new StringLengthExpression(s1))
    val i2: HelperExpression = HelperExpression("i2", zero, SelfExpression("i2") <= new StringLengthExpression(s2), new StringLengthExpression(s2))

    // what the compute() method calls with helper(s1.length(), s2.length())
    val parameters = Map("i1" -> new StringLengthExpression(s1), "i2" -> new StringLengthExpression(s2))
    val helpers = Map("i1" -> i1, "i2" -> i2)
    val sol = SubproblemInvocation(parameters, order=Seq("i1", "i2"), helpers = helpers, returnType = BooleanType())
    ...
    val case_5 = IfThenElseDefinition(i2 == zero,
      ExpressionStatement(new CharAtExpression(s1, i1 - one) == new CharAtExpression(s3, i1 - one)),
      ExpressionDefinition(case_final))

    val case_4 = IfThenElseDefinition(i1 == zero,
      ExpressionStatement(new CharAtExpression(s2, i2 - one) == new CharAtExpression(s3, i2 - one)),
      case_5)
