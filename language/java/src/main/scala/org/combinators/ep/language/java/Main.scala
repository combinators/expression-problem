...
object Main extends App {
  ...
  val evolutions = Seq(M0, M1, M2) // , M2, M3)

  evolutions.foreach { evolution =>

    val impl = approach.implement(
          evolution.getModel,
          eips.M2.apply(approach.paradigm)(generator.doublesInMethod, generator.stringsInMethod)
            .asInstanceOf[EvolutionImplementationProvider[approach.type]]
        )
