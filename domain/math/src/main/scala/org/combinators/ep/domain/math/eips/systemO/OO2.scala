...
object OO2 {
  ...
  EvolutionImplementationProvider[AIP[paradigm.type]] = {
    val oo2Provider: EvolutionImplementationProvider[AIP[paradigm.type]] = new EvolutionImplementationProvider[AIP[paradigm.type]] {
      override val model: GenericModel = math.systemO.OO2.getModel
      ...
      override def dependencies(potentialRequest: PotentialRequest): Option[Set[Operation]] = {
        ...
        if (cases.contains(potentialRequest.tpeCase)) {
          (potentialRequest.op, potentialRequest.tpeCase) match {
            case (math.systemO.OO2.InsertM, _) => Some(Set.empty)
            case (_, _) => None
          }
        } else {
          None
    ...
    // newest first
    monoidInstance.combine(oo2Provider, m2Provider)
  }
}
