...
import org.combinators.ep.domain.math.J8.{PowBy, PowByTestCase}
import org.combinators.ep.domain.math.M0.{DoubleInst, Eval, LitInst}
import org.combinators.ep.domain.{Evolution, GenericModel}

object J9 extends Evolution {
  override implicit def getModel:GenericModel = J5J8.getModel.evolve("j9", Seq(Inv), J2.isOps(Seq(Inv)))
  ...
  val all_instances = J5J8.all_instances ++ Seq(invi)
  val lhs           = J5J8.lhs ++ Seq(invi_same_lhs)   // changes on left hand side
  val rhs           = J5J8.rhs ++ Seq(invi_same_rhs)   // changes on right hand side
