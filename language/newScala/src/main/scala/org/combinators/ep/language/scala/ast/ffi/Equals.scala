...
trait Equals[AST <: EqualsAST & BaseAST, B <: org.combinators.cogen.paradigm.AnyParadigm, T: ClassTag](
  ...
  trait ScalaEqualsIn[Ctxt](val registry: ContextRegistry[B, Ctxt]) extends super.EqualsIn[Ctxt] {
  ...
  val equalsInMethods: ScalaEqualsIn[_base.ast.any.Method] = new ScalaEqualsIn(methodRegistry) {}
  val equalsInConstructors: ScalaEqualsIn[_base.ast.oo.Constructor] = new ScalaEqualsIn(constructorRegistry) {}
  val equalsInClasses: ScalaEqualsIn[_base.ast.oo.Class] = new ScalaEqualsIn(classRegistry) {}
}

object Equals {
  type WithBase[T, AST <: EqualsAST & BaseAST, B <: AnyParadigm.WithAST[AST]] = Equals[AST, B, T] {}
  def apply[T: ClassTag, AST <: EqualsAST & BaseAST, B <: AnyParadigm.WithAST[AST]](
    _base: B,
    matchingTpeRep: TypeRep.OfHostType[T],
    methodRegistry: ContextRegistry[B, _base.ast.any.Method],
    constructorRegistry: ContextRegistry[B, _base.ast.oo.Constructor],
    classRegistry: ContextRegistry[B, _base.ast.oo.Class],
  ): WithBase[T, AST, B] = new Equals[AST, B, T](_base, matchingTpeRep, methodRegistry, constructorRegistry, classRegistry) with Eql[AST, B, T](_base) {}
}
