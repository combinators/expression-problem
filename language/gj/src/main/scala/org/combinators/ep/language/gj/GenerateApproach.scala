...
abstract class BaseTest(val id:String) {
  ...
  def generatedCode(approachName:String, systemName: String): Long = {
    ...
    val outputDir = Paths.get("target", "ep-firstVersion", "gj", approachName, systemName)
    ...
    gj_code.foreach(u => {
      val path = Paths.get("target", "ep-firstVersion", "gj", approachName, systemName, u.persistTo.toString)
