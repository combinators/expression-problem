...
sealed class J5[P <: AnyParadigm, AIP[P <: AnyParadigm] <: ApproachImplementationProvider.WithParadigm[P], IfBlockType](val paradigm: P) {
  ...
  (ffiArithmetic: Arithmetic.WithBase[paradigm.MethodBodyContext, paradigm.type, Double],
   ...
   ifThenElse: IfThenElseCommand
    ...
    val j5Provider = new EvolutionImplementationProvider[AIP[paradigm.type]] {
      override val model = math.J5.getModel
      ...
      def applicable(forApproach: AIP[paradigm.type], potentialRequest:PotentialRequest): Boolean = {
        (Set(math.J5.Simplify).contains(potentialRequest.op) &&
          math.J4.getModel.flatten.typeCases.toSet.contains(potentialRequest.tpeCase)) ||
          Set(math.J5.Collect).contains(potentialRequest.op)  // any foreseeable DT can be collected
      }
                ...
                collectedResults <- forEach(onRequest.attributes.toSeq) {
                  ...
                  attExpr => {
                    ...
                    forApproach.dispatch(
                      SendRequest(
                        expr,
                        math.J5.getModel.baseDataType,
                        Request(math.J5.Collect, Map.empty),
                        Some(onRequest)
                      )
                    )}
                }
                  ...
                  result <-
                    ifThenElse(
                      ...
                      for {                                       // default
                        lsimp <- simplifyRec(atts.head, attExprs.head)
                        rsimp <- simplifyRec(atts.tail.head, attExprs.tail.head)
                        res <- returnInIf(forApproach.instantiate(math.M0.getModel.baseDataType, math.J4.Power, lsimp, rsimp))
                      } yield res
                    )
                } yield result
              }
      ...
      Generator[forApproach.paradigm.MethodBodyContext, Option[forApproach.paradigm.syntax.Expression]] = {
        onRequest.request.op match {
          case math.J5.Collect => collectLogic(forApproach)(onRequest)
      ...
      Generator[forApproach.paradigm.MethodBodyContext, Option[forApproach.paradigm.syntax.Expression]] = {
        ...
        onRequest.request.op match {
          case math.J5.Collect => genericLogic(forApproach)(onRequest)
...
object J5 {
  ...
  EvolutionImplementationProvider[AIP[paradigm.type]] = {
    ...
    val mkImpl = new J5[paradigm.type, AIP, Expression](paradigm)
  ...
  EvolutionImplementationProvider[AIP[paradigm.type]] = {
    ...
    val mkImpl = new J5[paradigm.type, AIP, Unit](paradigm)
