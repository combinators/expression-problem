...
import com.github.javaparser.ast.expr.{AssignExpr, CastExpr, EnclosedExpr, Expression, FieldAccessExpr, InstanceOfExpr, MethodCallExpr, NameExpr, ObjectCreationExpr, ThisExpr, TypeExpr, Name as JName}
      ...
      implicit val canAddImplementedInClass: Understands[ClassContext, AddImplemented[Type]] =
        new Understands[ClassContext, AddImplemented[Type]] {
          def perform(
            ...
            val toAdd = command.interface.asClassOrInterfaceType()
            val resultCls = context.cls.clone()
            if (!resultCls.getImplementedTypes.contains(toAdd)) {       // only add if not already present...
              resultCls.addImplementedType(toAdd)
            }
            (context.copy(cls = resultCls), ())
          }
        }
