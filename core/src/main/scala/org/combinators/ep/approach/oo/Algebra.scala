...
trait Algebra extends OOApproachImplementationProvider with BaseDataTypeAsInterface with SharedOO with FutureVisitor with OperationInterfaceChain with FieldDefinition with FactoryConcepts {
  ...
  /** For Algebra, the covariant type needs to be selected whenever a BaseType in the domain is expressed. */
...
object Algebra {
  type WithParadigm[P <: AnyParadigm] = Algebra { val paradigm: P }
  ...
  (generics: Generics.WithBase[base.type,oo.type,params.type]): Algebra.WithParadigm[base.type] =
    new Algebra {
