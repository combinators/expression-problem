# Process jacoco.* output generated by the compile.py scripts
# within the directory that was result of generating systems for different approaches. Note
...
def extract(A):
    ...
    for line in A:
        if '[error]' in line:
            hasError = 'error'
            continue
        if '[info]' in line and cov_stmts[0] in line:
            coverage_instructions = line.split()[cov_index]
        if '[info]' in line and cov_stmts[1] in line:
            coverage_branches = line.split()[cov_index]
