...
object MatrixChainMultiplicationMainBottomUpDirectToDiskMain extends IOApp {
  ...
  def model:EnhancedModel = {
    ...
    val c: HelperExpression = HelperExpression("c", two, SelfExpression("c") <= new ArrayLengthExpression(array) - one, new ArrayLengthExpression(array) + one)
    val r: HelperExpression = HelperExpression("r", one, SelfExpression("r") <= new ArrayLengthExpression(array) - c, new ArrayLengthExpression(array) + one)

    val k: HelperExpression = HelperExpression("k", r, SelfExpression("k") < (r + c - one), new ArrayLengthExpression(array)) // k will always be within this range

    val helpers = Map("c" -> c, "r" -> r)
    val mappers = Map("c" -> r, "r" -> (r + c - one))   // controls how problems are "mapped" into the dp[] design space
     ...
     *      for (int k = i; k < j; k++)
     */
    val subprobExpr = new SubproblemExpression(Seq(r, k)) + new SubproblemExpression(Seq(k + one, r + c - one)) + array(r - one) * array(k) * array(r + c - one)
