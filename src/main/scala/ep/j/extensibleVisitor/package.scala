...
public class Add extends Exp {
  ...
  *
  * The '''Eval''' operation is part of the first [[ep.domain.M0]] evolution, and its definition
  * is as follows. Note that it provides ''visit'' implementations for the '''Lit''' and '''Add'''
  * data-types, which are also defined in [[ep.domain.M0]]. The ''makeEval'' operation returns an
...
public class Eval implements Visitor<Double> {
  ...
  * In [[ep.domain.M1]] the '''Sub''' data-type is defined:
